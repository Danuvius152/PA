...
enum {
  TYPE_R, TYPE_I, TYPE_I_,TYPE_U, TYPE_J, TYPE_B, TYPE_S,
  TYPE_N, // none
};
...
#define immI_() do { *imm = SEXT(BITS(i, 25, 20), 6); } while(0)
...
static void decode_operand(Decode *s, int *dest, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *dest = rd;
  switch (type) {
    ...
    case TYPE_I_:src1R();          immI_(); break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I_, R(dest) = src1 << imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I_, R(dest) = src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I_, R(dest) = (int)src1 >> imm);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = (int)src1 >> (src2 & 31));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, long long x = (int)src1; long long y = (int)src2; R(dest) = (x * y) >> 32);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = ((int)src1 * (long long)src2) >> 32);
